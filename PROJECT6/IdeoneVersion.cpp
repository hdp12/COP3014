...
do{
	...
	cout << "WOULD YOU LIKE TO BUY ANOTHER BEVERAGE? (Y)ES OR (N)O..." << endl;
	cin >> leaveConfirm;
} while (leaveConfirm == 'y' || leaveConfirm == 'Y');
...
// IDEONE BEGIN
int sodas(listSodas drinks){
	for (int i=0; i<6; i++){
		cin >> drinks[i].drinkName;
		cin >> drinks[i].price;
		cin >> drinks[i].numAvailableCans;
	}
	return 0;
}
// IDEONE END
